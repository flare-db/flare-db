...
public interface FlattenExamplePipelineOptions extends FlarePipelineOptions {
   ...
   */
  static FlattenExamplePipelineOptions applyFlareDefaults(FlattenExamplePipelineOptions options) {
  ...
  private static File findShadowJar() {
    ...
    for (String dir : candidateDirs) {
      File[] matches =
          new File(dir)
              .listFiles((d, name) -> name.startsWith("flatten-") && name.endsWith("-all.jar"));
